# NoteMind — 企业级 Agentic RAG 智能知识中枢
...
```text
Browser (web / admin)                    L4 体验
        │  /api/**
        ▼
notemind-gateway :8080          ← Java   L3 网关（语义缓存 / 意图路由在 Service 编排）
        │
        ▼
notemind-service :8081          ← Java   鉴权 / MySQL CRUD / 编排
        │  AiEngineClient
        ▼
backend (AI Engine) :8000       ← Python L2 检索 + LangGraph Agent
        │
        ▼
MySQL + PGVector（规划 Neo4j）           L1 数据底座（父子索引已落地）
```
...
用户管理；**系统配置**（高频默认值：默认知识库/模型/策略、分页、上传扩展、Prompt 场景、TopK、温度、会话 LIMIT，Admin `/system/config`）；知识库与策略；文档上传后触发 AI 入库；片段修正；检索测试 / 召回调试台；模型与 Prompt；工具中心；Agent 执行与时间线；问答应用；对话日志；评测集 / 批量评测 / 对比看板；**意图路由 / 语义缓存 / 图谱检索**测试页。
