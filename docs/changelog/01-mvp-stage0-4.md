...
- `LoginRateLimiter`：按 IP + 用户名统计失败；默认用户 5 次/IP 30 次（5 分钟窗口）锁定 15 分钟
- 登录成功清用户名计数；超限 HTTP 429；本机扫用户名不会再误锁 admin

## 2026-09-22（深度扫描收口）

- 种子账号：停用后不再被启动/登录种子逻辑自动启用
- `DevSecretGuard`：内置 `changeme`/`changeme` 与 JWT/AI Token 同级，无弱默认开关则拒启
- `GET /api/v1/qa-apps` 登录即可（启用列表）；`/page`/`/{id}` 等管理接口需 ADMIN
- 问答 SSE：会话须同时归属当前用户且 `app_id` 匹配；Admin/Web 流式 401/403 清本地 Token
- 语义缓存余弦：向量维度不一致直接判 0（禁止截断对齐）
- 文档/知识库/片段：删清向量失败显式报错；**先清向量再软删**（可重试）；切分/重传清向量失败中止；文档 `EMBEDDING` 超时 2h 自动回收；停用应用禁止 SSE
- 删知识库级联停用挂载的问答应用；JWT 每次校验库内停用/角色
- Gateway 默认绑 `127.0.0.1`（`GATEWAY_ADDRESS=0.0.0.0` 可放开）
- 出站 SSRF：Java/Python 拦截 CGNAT/`fc00::/7`；非弱默认仅 `*.aliyuncs.com` 白名单
- AI Engine 启动校验默认 `AI_ENGINE_TOKEN`（需 `NOTEMIND_ALLOW_WEAK_DEFAULTS`）
- 语义缓存测试接口 `lookup/clear/stats` 收紧为 ADMIN（防任意登录用户刷 Embedding）
- 改密后旧 JWT 通过密码指纹立即失效；KB 向量状态不再用 meta 伪装 READY；文档批删 CONFLICT 显式报错
- 删知识库级联清理 upload-dir 内文件；SSE 自动建会话透传 clientSource；Admin 批删不再吞 502/409
- `AI_ENGINE_URL`：非本机禁止明文 http；生产拒绝默认 DB 口令；模型出站始终白名单；AI Engine `/docs` 仅弱默认开启

## 2026-09-22（第三轮复扫）

- Python OpenAI 兼容客户端统一 `httpx(follow_redirects=False)` + `url_safety`（chat/embed）
- `AiEngineHttpClient` 禁止跟随重定向；Agent 错误不再回传内部异常串
- Gateway 显式 `GATEWAY_ADDRESS`（默认 `0.0.0.0` 供本机浏览器）

## 2026-09-22（复扫补丁）

- HttpClient 禁止跟随重定向，防 SSRF 跳转内网
- 文档删除/批量删在 EMBEDDING 中拒绝；本地删文件走 upload-dir jail
- 评测 `isCancelled` 库异常时 fail-open，避免误杀任务
- `start-*.bat` 不再内嵌百炼 Key，改为从 `.env` 读取；Swagger 仅弱默认开启

## 2026-09-22（安全与逻辑加固）

- 默认 JWT/AI Token：无 `NOTEMIND_ALLOW_WEAK_DEFAULTS` 时拒绝启动；生产禁止弱默认；建议独立 `NOTEMIND_API_KEY_SECRET`
- 出站 SSRF：`OutboundUrlGuard` / `url_safety` 校验模型 `baseUrl` 与 LLM 代理；禁私网；AI Engine 关闭浏览器 CORS
- Service 默认 `server.address=127.0.0.1`（`SERVER_ADDRESS` 可覆盖）
- 文档 `file_path` 必须落在 upload-dir；向量化不再先删后写；`EMBEDDING` 中禁清向量/切分/重传
- 语义缓存仅写入成功有据回答；Chat 无登录抛 401
- 评测/Agent `RUNNING` 超时（2h）自动回收；评测取消落终态
- 全局异常不再回传内部堆栈细节；multipart 文件名滤 CR/LF

## 2026-09-20（残留收口：手工写缓存 + demo 停用修复）

- 手工 `/cache/store` 与 `NOTEMIND_ALLOW_WEAK_DEFAULTS` 解耦，改由 `NOTEMIND_ALLOW_MANUAL_CACHE_WRITE`（默认 false）；仅 ADMIN；答案长度上限
- `clear` 强制有效 userId，禁止跨用户清理；lookup/clear/stats/intent 登录即可（按 userId 隔离）
- 种子账号若被停用（status=0）启动/登录时自动恢复启用（不覆盖密码）

## 2026-09-20（Phase B/C 安全加固复扫）

- 语义缓存：按 `userId|appId|kbId` 隔离；手工 `/cache/store` 默认关闭（`NOTEMIND_ALLOW_MANUAL_CACHE_WRITE`）；`clear` 必须带 appId 或 kbId；Redis ping try-with-resources；问题长度/扫描上限；Embedding 读系统配置模型
- 知识图谱：`overview` 不再返回本机 `storePath`；多跳 `visited` + `MAX_PATHS=50` + depth≤3；`KgMultiHopRequest` VO
- 删除死代码 `IntentRoutePlan`；Chat 问答落缓存带当前用户；测试页提示手工写入禁用原因

## 2026-09-20（Day2～Day4：Agentic/评测 + 意图缓存 + 图谱可测）

- **Day2**：Agent 执行 SUCCESS（5 步）；评测集「开工验收-续航散热」10 用例；批量报告 baseline/full 等已 COMPLETED
- **Day3**：意图路由 `IntentRouteAsvc` + `/ai/intent-test`；语义缓存 Redis（精确/余弦）+ `/ai/cache-test`；问答 SSE 增加 `meta` 过程面板（意图/缓存/管线）
- **Day4**：本地 JSON 知识图谱 `KnowledgeGraphAsvc` + `/knowledge/kg-test`（实体检索/多跳，不依赖 Neo4j）
- 问答二次同问可演示缓存命中（`pipeline=cache`）

## 2026-09-20（开工 Day0～Day1 联调验收）

- 启动本机 MySQL + Gateway/Service/AI/Admin；同步 `key.txt` 中百炼 Key 至 `.env` 与模型配置（含新增 `qwen-max`）
- 模型连通：`qwen-plus` / `text-embedding-v4` / `gte-rerank-v2` / `qwen-max` 均「对话/向量/重排接口连通正常」
- 数据闭环：上传 `scripts/_e2e_kickoff_sample.md` → 切分 → 向量化 READY → 检索命中 1 → SSE 问答有 sources，回答含「18 小时」「硬件评测中心」
- 可测入口不变；密钥仅本地 `key.txt` / `.env` / 库内加密字段，不入库文档

## 2026-09-20（开工手册：百炼 Key · 测试模块 · 流程）

- 新增 `docs/plan/02-kickoff-bailian-and-test-modules.md`：① 百炼仅需 1 个 API Key + 三类模型；Phase B/C 追加小模型配置 ② 测试页 P0 增强与 P1/P2 新页 ③ Day0～Day4 开工流程
- README 文档表增加该手册入口

## 2026-09-20（企业级 Agentic RAG 架构对齐）

- 吸收 `docs/castom` 原文方案（治本/降本/增效、四层架构），以 NoteMind 双栈为基线整理**详细技术说明书**：`docs/architecture/02-enterprise-agentic-rag.md`
- 更新 `01-overview.md`、`docs/rag/01-pipeline.md`、`docs/plan/01-development-plan.md`、`README.md`、`开发约定文档` 四层映射与 Phase A/B/C
- 方案补充稿：`docs/castom/01-enterprise-agentic-rag-supplemented.md`
- 架构占位（不伪装已实现）：Python `infrastructure/cache/semantic_cache.py`、`infrastructure/kg/`（Java 侧语义缓存/意图/本地图谱已可测，见后续变更）
