...
> **企业级说明书**：[架构 · 02](../architecture/02-enterprise-agentic-rag.md)（四层 + 能力成熟度）  
> 当前状态：MVP 阶段 0～4 主链路与运营后台已可测；Phase B（语义缓存/意图路由测试页）与 Phase C（本地 JSON 图谱测试页）最小可测已落地；大小模型按节点分流与 Neo4j 仍规划中
...
| 阶段 | 目标 | 状态摘要 |
| --- | --- | --- |
| **Phase A** 基础 RAG 与数据治理 | 父子索引、重排、有据问答 | ✅ 主体完成；Wiki/HTML 深度清洗可加强 |
| **Phase B** 路由分发与成本优化 | 语义缓存、L1/L2/L3、大小模型协同 | 🟡 Java `SemanticCacheAsvc` / `IntentRouteAsvc` + Admin 测试页可测；大小模型按节点分流仍规划 |
| **Phase C** Agentic 与图谱增强 | Neo4j Tool、多源并行检索 | 🟡 LangGraph 已有；本地 JSON 图谱 `/knowledge/kg-test` 可测；Neo4j / 多源并行仍规划 |
