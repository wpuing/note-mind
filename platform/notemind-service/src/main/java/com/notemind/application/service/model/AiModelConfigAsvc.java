...
public class AiModelConfigAsvc {
    ...
    public AiModelConfigAsvc(JdbcTemplate jdbcTemplate, ObjectMapper objectMapper, ApiKeyCrypto apiKeyCrypto) {
    ...
    private AiModelConfigTestResult testEmbedding(AiModelConfigVo vo, String apiKey, long t0)
            throws Exception {
        String base = trimSlash(blankToDefault(
        ...
        return AiModelConfigTestResult.fail(
                "向量接口返回 " + resp.statusCode() + ": " + abbreviate(resp.body()),
                resp.statusCode(),
                ms);
    }
    ...
    private AiModelConfigTestResult testChat(AiModelConfigVo vo, String apiKey, long t0)
        ...
        return AiModelConfigTestResult.fail(
                "对话接口返回 " + resp.statusCode() + ": " + abbreviate(resp.body()),
                resp.statusCode(),
                ms);
    }
    ...
    private AiModelConfigTestResult testRerank(AiModelConfigVo vo, String apiKey, long t0)
        ...
        return AiModelConfigTestResult.fail(
                "重排接口返回 " + resp.statusCode() + ": " + abbreviate(resp.body()),
                resp.statusCode(),
                ms);
    }
