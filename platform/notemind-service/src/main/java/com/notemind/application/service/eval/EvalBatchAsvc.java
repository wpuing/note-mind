...
public class EvalBatchAsvc {
    ...
    private boolean isCancelled(String reportId) {
        ...
        try {
            ...
            // 瞬时库故障勿误判为取消
            log.warn("isCancelled check failed reportId={}: {}", reportId, e.getMessage());
            return false;
        }
    }

    /** 将超时仍 RUNNING 的任务标记 FAILED，避免永久挡新开跑。 */
    private void reclaimStaleRunning(Duration maxAge) {
        Timestamp cutoff = Timestamp.valueOf(LocalDateTime.now().minus(maxAge));
        int n = jdbcTemplate.update(
                """
                UPDATE t_eval_report
                SET status = 'FAILED',
                    fail_reason = '任务超时未完成（进程中断或卡死），已自动回收',
                    update_time = ?
                ...
                cutoff);
        if (n > 0) {
            log.warn("reclaimed {} stale RUNNING eval report(s)", n);
        }
    }

    private void markCancelled(String reportId, long durationMs) {
        jdbcTemplate.update(
                """
                UPDATE t_eval_report
                SET status = 'FAILED', fail_reason = '已取消', duration_ms = ?, update_time = ?
    ...
    public EvalReportVo start(EvalBatchStartRequest req) {
        ...
        synchronized (startLock) {
            reclaimStaleRunning(Duration.ofHours(2));
        ...
        try {
            ...
            for (int i = 0; i < cases.size(); i++) {
                ...
                if (isCancelled(reportId)) {
                    ...
                    markCancelled(reportId, System.currentTimeMillis() - t0);
                    return;
                }
