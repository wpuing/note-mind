...
public class KnowledgeBaseAsvc {
    ...
    public int batchDelete(List<String> ids) {
        ...
        for (String id : ids) {
            ...
            // 软删除失败（不存在）则跳过
            if (!knowledgeBaseDsvc.softDeleteCascade(trimmed)) continue;
            // 尝试执行
            try {
                // 调用 AI 清理该库向量
