...
public class JdbcKnowledgeBaseRepository implements KnowledgeBaseRepository {
    ...
    /**
     * 注入 JdbcTemplate 与上传目录配置。
     */
    public JdbcKnowledgeBaseRepository(JdbcTemplate jdbcTemplate, StorageProperties storageProperties) {
    ...
    public boolean softDeleteCascade(String id) {
        ...
        // 先收集落盘路径，软删后按 upload-dir jail 清理
        List<String> filePaths = jdbcTemplate.query(
                """
                SELECT file_path FROM t_knowledge_document
                WHERE knowledge_base_id = ? AND deleted = 0
                  AND file_path IS NOT NULL AND file_path <> ''
                """,
                (rs, n) -> rs.getString("file_path"),
                id);
        int updated = jdbcTemplate.update(
                """
                UPDATE t_knowledge_base
        ...
        jdbcTemplate.update(
                ...
                WHERE knowledge_base_id = ? AND deleted = 0 AND enabled <> 0
                """,
                now,
                id);
        for (String fp : filePaths) {
            deleteUploadFileQuietly(fp);
        }
        return true;
    }

    /** 仅删除 upload-dir 内文件；越界忽略。 */
    private void deleteUploadFileQuietly(String filePath) {
        ...
        try {
            ...
            if (parent != null && Files.isDirectory(parent) && parent.startsWith(rootReal)) {
    ...
    @Override
    public long countEmbeddingDocuments(String knowledgeBaseId) {
        Long n = jdbcTemplate.queryForObject(
                """
                SELECT COUNT(1) FROM t_knowledge_document
                WHERE deleted = 0 AND knowledge_base_id = ?
                  AND UPPER(parse_status) = 'EMBEDDING'
                """,
                Long.class,
                knowledgeBaseId);
        return n == null ? 0L : n;
